...
function Emergency({ postId, date, severity, timestamp, urgency, victims, solved, fromId, address }) {
    ...
    const updateSolved = (e) => {
      e.preventDefault()
      if(window.confirm(`Are you sure you want to update: ${date} - ${severity} - ${urgency} -> From ${senderInfo?.name} \n\n As completed task?`)){
        db.collection("emergency-response").doc(postId).update({
          solved:true,
          read:false
          }).then(function() {
            toast.success("Emergency has been updated as completed task!");
          });
      }     
     }

    const deleteEmergency = () =>{
      if(window.confirm(`Are you sure you want to delete: ${date} - ${severity} - ${urgency} -> From ${senderInfo?.name}?`)){
          db.collection("emergency-response").doc(postId).delete().then(function() {
          }).catch(function(error) {
              toast.error("Error removing post: ", error);
          }); 
          toast.success("Emergency has been deleted!")   
        }
  }
